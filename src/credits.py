...
def render_credit(console, context, anim_x:int, anim_y:int, anim_frame: int) -> None:
    ...
    render_credit_gui(console=console)
    render_credit_animation(console=console, x=anim_x, y=anim_y, frame=anim_frame)
...
def credit_event_handler(console, context):
    ...
    animation_frame = 1
    render_credit(console, context, credit_animation_x, credit_animation_y, animation_frame)

    # credit screen loop
    while True:
        # Get Input from credit + timeoout when this animation frame is overs next animation
        credit_action = get_credit_action(sec_per_frame=sec_per_frame)
        ...
        render_credit_animation(console=console, x=credit_animation_x, y=credit_animation_y, frame=animation_frame)
