...
def draw_circle(engine, center_x: int, center_y: int, radius: int, fat_circles: bool = False, penetrate_wall: bool = True, only_in_sight: bool = False) -> List[Tuple(int,int)]:
    """
    ...
    for horizontal in grid:
        for xy in horizontal:
            map_x = min(max(xy[0] + center_x, 0), engine.game_map.width)
            map_y = min(max(xy[1] + center_y, 0), engine.game_map.height)
